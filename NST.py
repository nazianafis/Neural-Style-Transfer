...
# ---Image manipulation functions begin

def load_image(img_path,target_shape="None"):
    if not os.path.exists(img_path):
        raise Exception(f'Path does not exist: {img_path}')
    img = cv.imread(img_path)[:, :, ::-1]  # [:, :, ::-1] converts BGR (OpenCV's default format) into RGB
    ...
    # this needs to go after resizing - otherwise cv.resize will push values outside of [0,1] range
    img = img.astype(np.float32)  # convert from uint8 to float32
    img /= 255.0  # convert to [0, 1] range
    return img

def prepare_img(img_path, target_shape, device):
    img = load_image(img_path, target_shape=target_shape)
    # normalize using ImageNet's mean
    # [0, 255] range works better than [0, 1] range (even though PyTorch models were trained on the latter)
...
def save_image(img, img_path):
    ...
    cv.imwrite(img_path, img[:, :, ::-1])  # [:, :, ::-1] converts RGB into BGR (OpenCV's default format) 
...
def prepare_model(device):
    ...
    # we are not tuning model weights -> we are only tuning optimizing_image's pixels. (That's why requires_grad is set to False)
...
# Builds a function that performs a step in tuning loop
...
def neural_style_transfer(config):
    ...
    def closure():
        ...
        with torch.no_grad():
            ...
            save_and_maybe_display(optimizing_img, dump_path, config, cnt, num_of_iterations[config['optimizer']])
...
default_resource_dir = os.path.join('/home/nn007/Desktop/myAssignments/DL/DL-project', 'data')
content_images_dir = os.path.join(default_resource_dir, 'content-images')
style_images_dir = os.path.join(default_resource_dir, 'style-images')
output_img_dir = os.path.join(default_resource_dir, 'output-images')
img_format = (4, '.jpg')  # saves images in the format: %04d.jpg

optimization_config = {'content_img_name': 'c1.jpg', 'style_img_name': 's6.jpg', 'height': 400, 'content_weight': 100000.0, 'style_weight': 30000.0, 'tv_weight': 1.0, 'optimizer': 'lbfgs', 'model': 'vgg19', 'init_method': 'content', 'saving_freq': -1}
